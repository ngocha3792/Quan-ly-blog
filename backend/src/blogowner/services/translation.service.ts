...
export class TranslationService {
  ...
  async translatePost(input: TranslatePostInput): Promise<{
    title: string;
    content: string;
  }> {
    ...
    if (!baseUrl) {
      throw new ServiceUnavailableException('Dịch tự động chưa được cấu hình.');
    }

    const sourceLanguageCode = this.normalizeLanguageCode(
      input.sourceLanguageCode,
    );

    const targetLanguageCode = this.normalizeLanguageCode(
    ...
    if (
      ...
      typeof result.translatedText[0] !== 'string' ||
      typeof result.translatedText[1] !== 'string'
    ) {
      throw new BadGatewayException(
        'Dịch vụ dịch tự động trả về dữ liệu không hợp lệ.',
      );
