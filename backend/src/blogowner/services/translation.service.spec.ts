...
describe('TranslationService', () => {
  ...
  it('should translate title and content with LibreTranslate', async () => {
    ...
    const json = jest.fn().mockResolvedValue({
      translatedText: ['NestJS Guide', '<p>English content</p>'],
    });
    ...
    const result = await service.translatePost({
    ...
    expect(mockConfigService.get).toHaveBeenCalledWith('TRANSLATE_API_URL');

    expect(mockFetch).toHaveBeenCalledWith('http://localhost:5000/translate', {
      method: 'POST',
      ...
      body: JSON.stringify({
        q: ['Hướng dẫn NestJS', '<p>Nội dung tiếng Việt</p>'],
  ...
  it('should trim and lowercase language codes before calling LibreTranslate', async () => {
    ...
    mockFetch.mockResolvedValue({
      ...
      json: jest.fn().mockResolvedValue({
        translatedText: ['Hello', '<p>English content</p>'],
      }),
    });
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        body: JSON.stringify({
          q: ['Xin chào', '<p>Nội dung tiếng Việt</p>'],
  ...
  it('should remove trailing slash from TRANSLATE_API_URL', async () => {
    ...
    mockFetch.mockResolvedValue({
      ...
      json: jest.fn().mockResolvedValue({
        translatedText: ['Hello', '<p>Content</p>'],
      }),
    });
  ...
  it('should throw ServiceUnavailableException when TRANSLATE_API_URL is missing', async () => {
    ...
    await expect(
      ...
      new ServiceUnavailableException('Dịch tự động chưa được cấu hình.'),
    );
  ...
  it('should throw BadGatewayException when LibreTranslate cannot be reached', async () => {
    ...
    await expect(
      ...
      new BadGatewayException('Không thể kết nối tới dịch vụ dịch tự động.'),
    );
  });
  ...
  it('should throw BadRequestException when LibreTranslate returns status 400 with an error message', async () => {
    ...
    const json = jest.fn().mockResolvedValue({
      error: 'vi is not available as a source language',
    });
  ...
  it('should throw BadGatewayException when LibreTranslate returns non-OK response', async () => {
    ...
    await expect(
      ...
      new BadGatewayException('Dịch vụ dịch tự động không thể xử lý yêu cầu.'),
    );
  });
  ...
  it('should throw BadGatewayException when LibreTranslate returns invalid data', async () => {
    ...
    mockFetch.mockResolvedValue({
      ...
      json: jest.fn().mockResolvedValue({
        translatedText: 'Only one string',
      }),
    });
  ...
  it('should throw BadGatewayException when translatedText array is incomplete', async () => {
    ...
    mockFetch.mockResolvedValue({
      ...
      json: jest.fn().mockResolvedValue({
        translatedText: ['Only translated title'],
      }),
    });
