...
export class BlogownerPostsService {
  ...
  async findAll(
    ...
    const candidateRootIds = Array.from(
      new Set(matchingPosts.map((post) => post.parentPostId ?? post.id)),
    );
    ...
    const sortedMetrics = Array.from(groupedMetrics.values()).sort((a, b) => {
      let comparison = 0;
      ...
      if (sortBy === 'viewCount') {
        ...
        comparison = a.latestUpdatedAt.getTime() - b.latestUpdatedAt.getTime();
      }
     ...
     */
    const pageRoots = await this.prisma.post.findMany({
      where: {
        id: {
          in: pageRootIds,
        },
     ...
     * đã tính ở phía trên.
     */
    const rootById = new Map<number, BlogownerPostEntity>();

    for (const rawRoot of pageRoots) {
      rootById.set(rawRoot.id, new BlogownerPostEntity(rawRoot));
    }
  ...
  async remove(
     ...
     * Resolve group trước.
     */
    const { root } = await this.helper.findOwnedPostGroup(ownerId, postId);
    ...
    return {
      message: `Đã xóa bài viết ID ${root.id} và tất cả bản dịch.`,
    };
  }
  ...
  async submitForReview(
     ...
     */
    for (const groupPost of [root, ...translations]) {
      this.helper.assertSubmittable(groupPost.status);
    }
