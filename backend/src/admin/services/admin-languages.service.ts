...
export class AdminLanguagesService {
  ...
  async createLanguage(
    ...
    try {
      const language = await this.prisma.$transaction(async (tx) => {
        const currentDefault = await tx.language.findFirst({
          where: {
            isDefault: true,
            deletedAt: null,
          },
        ...
        if (shouldBeDefault && dto.isActive === false) {
        ...
        return this.languagesService.create(createDto, tx);
      });

      return new AdminLanguageEntity(language);
    } catch (error) {
      this.rethrowLanguageUniqueViolation(error, dto.code);
  ...
  async findAllLanguages(): Promise<AdminLanguageEntity[]> {
    ...
    return languages.map((language) => new AdminLanguageEntity(language));
  }
  ...
  async updateLanguage(
    ...
    try {
      const updated = await this.prisma.$transaction(async (tx) => {
        const current = await this.languagesService.findOne(id, tx);
         ...
         * Default luôn phải active.
         */
        if (current.isDefault && dto.isActive === false) {
          throw new DefaultLanguageCannotBeDeactivatedException();
        }

        const willBecomeDefault = dto.isDefault === true;
         ...
         * trạng thái hiện tại.
         */
        const resultingIsActive = dto.isActive ?? current.isActive;
         ...
         * => không cho phép.
         */
        if (willBecomeDefault && !resultingIsActive) {
          throw new DefaultLanguageMustBeActiveException();
        }
  ...
  async deleteLanguage(id: number): Promise<AdminLanguageEntity> {
    const deleted = await this.prisma.$transaction(async (tx) => {
      ...
      return this.languagesService.remove(id, tx);
    });
   ...
   */
  private rethrowLanguageUniqueViolation(error: unknown, code?: string): void {
    if (
     ...
     */
    if (targetText.includes('code') && code) {
