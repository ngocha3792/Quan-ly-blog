...
export class CleanupService {
  ...
  async handleCron() {
    this.logger.log(
      'Bắt đầu dọn dẹp dữ liệu xóa mềm (Soft-deleted) quá 30 ngày...',
    );
  ...
  private async cleanupMedia(thresholdDate: Date) {
    ...
    if (mediaToDelete.length > 0) {
      this.logger.log(
        `Tìm thấy ${mediaToDelete.length} media cần xóa vĩnh viễn.`,
      );
      ...
      for (const media of mediaToDelete) {
        if (media.publicId) {
          try {
            ...
            this.logger.error(
              `Không thể xóa ảnh trên Cloudinary với publicId: ${media.publicId}`,
              error,
            );
      ...
      this.logger.log(
        `Đã xóa vĩnh viễn ${deleted.count} bản ghi Media trong DB.`,
      );
    }
  }
  ...
  private async cleanupOtherTables(thresholdDate: Date) {
    ...
    for (const table of tables) {
      ...
      if (deleted.count > 0) {
        this.logger.log(
          `Đã xóa vĩnh viễn ${deleted.count} bản ghi trong bảng ${table.name}.`,
        );
      }
    }
  }
