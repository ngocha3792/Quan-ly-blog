...
export class AuthsService {
  ...
  async login(loginDto: LoginDto, ipAddress?: string, deviceInfo?: string) {
    ...
    const isMatch = await this.bcryptUtil.comparePassword(
      loginDto.password,
      user.passwordHash,
    );
    if (!isMatch) {
    ...
    const tokens = this.jwtUtil.generateToken(
      user.id.toString(),
      user.role,
      user.email,
    );

    const refreshTokenHash = await this.bcryptUtil.hashPassword(
      tokens.refreshToken,
    );
  ...
  async refreshToken(
    ...
    const payload = this.jwtUtil.verifyRefreshToken(
