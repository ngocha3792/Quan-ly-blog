...
import { CategoryGroupAlreadyExistsException, CategoryGroupNotFoundException } from '@app/core/common/exceptions';
...
export class CategoryGroupsService {
    ...
    async findAll(query: GetCategoryGroupsDto, paginationParams: PaginationParams, include?: Prisma.CategoryGroupInclude): Promise<PaginatedResult<CategoryGroupEntity>> {
        const { search } = query;
    ...
    async update(id: number, updateCategoryGroupDto: UpdateCategoryGroupDto) {
        ...
        if (updateCategoryGroupDto.code) {
            ...
            if (existingGroup) {
                throw new CategoryGroupAlreadyExistsException(updateCategoryGroupDto.code);
            }
        }
