...
describe('PostsService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a post without tags', async () => {
      ...
      expect(mockPrismaService.tag.createMany).not.toHaveBeenCalled();
    ...
    it('should resolve tags and create a post with postTags', async () => {
      ...
      mockPrismaService.tag.createMany.mockResolvedValueOnce({
        count: 1,
      });
      ...
      expect(mockPrismaService.tag.findMany).toHaveBeenNthCalledWith(1, {
        where: {
          id: {
            in: [1],
          },
      ...
      expect(mockPrismaService.tag.findMany).toHaveBeenNthCalledWith(2, {
        where: {
          name: {
            in: ['new-tag'],
          },
      ...
      expect(mockPrismaService.tag.findMany).toHaveBeenNthCalledWith(3, {
        where: {
          name: {
            in: ['new-tag'],
          },
    ...
    it('should reject a tag ID that does not exist or was soft-deleted', async () => {
      ...
      await expect(service.create(authorId, createDto)).rejects.toThrow(
        new BadRequestException('Có thẻ không tồn tại hoặc đã bị xóa.'),
      );
    ...
    it('should reject a tag name that belongs to a soft-deleted tag', async () => {
      ...
      await expect(service.create(authorId, createDto)).rejects.toThrow(
        new BadRequestException('Không thể sử dụng thẻ đã bị xóa: old-tag.'),
      );
