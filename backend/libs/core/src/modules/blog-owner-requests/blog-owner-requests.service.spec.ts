...
describe('BlogOwnerRequestsService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw ExistActionNotAllowedException if user has a pending request', async () => {
      mockPrismaService.blogOwnerRequest.findFirst.mockResolvedValueOnce({
        id: 1,
    ...
    it('should create a new request successfully if no pending requests exist', async () => {
      ...
      mockPrismaService.blogOwnerRequest.create.mockResolvedValueOnce(
        createdRequest,
      );
  ...
  describe('findAll', () => {
    it('should return a paginated result', async () => {
      ...
      const requests = [
        { id: 1, userId: 1, status: BlogOwnerRequestStatus.PENDING },
      ];

      mockPrismaService.blogOwnerRequest.findMany.mockResolvedValueOnce(
        requests,
      );
  ...
  describe('update', () => {
    ...
    it('should throw BlogOwnerRequestNotFoundException if request not found', async () => {
      mockPrismaService.blogOwnerRequest.findUnique.mockResolvedValueOnce(null);
      await expect(service.update(999, reviewerId, updateDto)).rejects.toThrow(
        BlogOwnerRequestNotFoundException,
      );
    });
    ...
    it('should update request if status is not APPROVED', async () => {
      ...
      mockPrismaService.blogOwnerRequest.update.mockResolvedValueOnce(
        updatedRequest,
      );
  ...
  describe('remove', () => {
    ...
    it('should delete and return the request if found', async () => {
      ...
      mockPrismaService.blogOwnerRequest.delete.mockResolvedValueOnce({
        id: 1,
      });

      const result = await service.remove(1);

      expect(prisma.blogOwnerRequest.delete).toHaveBeenCalledWith({
        where: { id: 1 },
      });
      expect(result.id).toEqual(1);
    });
