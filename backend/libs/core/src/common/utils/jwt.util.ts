...
export class JWTUtil {
  ...
  generateToken(userId: string, role: string, email: string) {
    ...
    if (!accessTokenSecretKey) {
      throw new InternalServerErrorException(
        'Thiếu cấu hình JWT_ACCESS_TOKEN_SECRET trong file .env',
      );
    }
    if (!refreshTokenSecretKey) {
      throw new InternalServerErrorException(
        'Thiếu cấu hình JWT_REFRESH_TOKEN_SECRET trong file .env',
      );
    }
  ...
  private handleJwtError(error: any, tokenType: string): never {
    ...
    throw new UnauthorizedException(
      `${tokenType} không hợp lệ hoặc đã bị chỉnh sửa.`,
    );
  }
}
