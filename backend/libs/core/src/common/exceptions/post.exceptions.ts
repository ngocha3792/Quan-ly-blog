...
export class PostAlreadyPublishedException extends BadRequestException {
  constructor(postId: string) {
    super(
      `Bài viết (ID: ${postId}) đã được xuất bản trước đó. Không thể thực hiện lại thao tác này.`,
    );
  }
}
...
export class PostNotFoundException extends NotFoundException {
  constructor(identifier: string) {
    ...
    super(
      `Không tìm thấy bài viết với định danh: ${identifier}. Bài viết có thể đã bị xóa hoặc ẩn.`,
    );
  }
}
...
export class TagLimitExceptions extends BadRequestException {
  constructor(limit: number) {
    super(
      `Mỗi bài viết chỉ được gắn tối đa ${limit} thẻ (tags). Vui lòng xóa bớt thẻ và thử lại.`,
    );
  }
}
