...
export class EmailAlreadyExistsException extends ConflictException {
  constructor(email: string) {
    super(
      `Địa chỉ email "${email}" đã được sử dụng. Vui lòng chọn một email khác hoặc đăng nhập.`,
    );
  }
}
...
export class SelfActionNotAllowedException extends BadRequestException {
  constructor(action: string) {
    ...
    super(
      `Bạn không thể tự thực hiện hành động "${action}" lên chính tài khoản hoặc nội dung của mình.`,
    );
  }
}
...
export class UserNotFoundException extends NotFoundException {
  constructor(identifier: string) {
    ...
    super(
      `Không tìm thấy người dùng: ${identifier}. Tài khoản có thể không tồn tại hoặc đã bị xóa.`,
    );
  }
}
...
export class UsernameAlreadyExistsException extends ConflictException {
  constructor(username: string) {
    super(
      `Tên người dùng "${username}" đã được sử dụng. Vui lòng chọn một tên người dùng khác hoặc đăng nhập.`,
    );
  }
}
