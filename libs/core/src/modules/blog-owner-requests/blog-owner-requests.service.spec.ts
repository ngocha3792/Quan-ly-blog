...
describe('BlogOwnerRequestsService', () => {
    ...
    describe('update', () => {
        ...
        it('should update request and not update user role if status is not APPROVED', async () => {
        ...
        it('should update request and update user role to BLOG_OWNER if status changed to APPROVED', async () => {
            const existingRequest = { id: 1, userId: 1, status: BlogOwnerRequestStatus.PENDING };
            mockPrismaService.blogOwnerRequest.findUnique.mockResolvedValueOnce(existingRequest);
